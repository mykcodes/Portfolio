...
class _ConstellationBackgroundState extends State<ConstellationBackground> with SingleTickerProviderStateMixin {
  ...
  // NEW: Blueprint grid planes
  late List<_BlueprintPlane> _blueprintPlanes;
  // NEW: Parametric curves
  ...
  void _generateLayers(Size size) {
    // 1. Deep Space (Many tiny slow stars)
    _deepSpaceNodes = List.generate(150, (index) {
      return _StarNode(
        basePosition: Offset(_random.nextDouble() * size.width, _random.nextDouble() * size.height * 2),
    ...
    // 2. Constellation (The core network)
    ...
    // 3. Particles (Fast moving energy sparks)
    ...
    // 4. Geometry (Wireframes, crosshairs)
    _geometryNodes = List.generate(15, (index) {
      return _GeometryNode(
        basePosition: Offset(_random.nextDouble() * size.width, _random.nextDouble() * size.height * 2),
        driftSpeed: _random.nextDouble() * 0.3 + 0.1,
        driftPhase: _random.nextDouble() * pi * 2,
        type: index % 3, // 0: crosshair, 1: square, 2: circle
        size: _random.nextDouble() * 40.0 + 20.0,
      );
    });

    // 5. Blueprint Planes (floating CAD-like wireframe rectangles at different depths)
    ...
    // 6. Parametric Curves (mathematical structures floating in void)
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: Listenable.merge([ExperienceController.instance.scrollController, _ambientController, ExperienceController.instance]),
      builder: (context, child) {
        final double globalIntensity = ExperienceController.instance.ambientIntensity;
        ...
        final String activeSection = ExperienceController.instance.activeSection;
        ...
        if (targetMouse != Offset.zero) {
          if (_smoothCursor == Offset.zero) {
            ...
            _smoothCursor = Offset.lerp(_smoothCursor, targetMouse, 0.08) ?? targetMouse;
          }
        }
        
        // We get maxScroll dynamically if available, otherwise estimate
        double maxScroll = 5000;
        if (ExperienceController.instance.scrollController.hasClients) {
          maxScroll = ExperienceController.instance.scrollController.position.maxScrollExtent;
        }
        ...
        return CustomPaint(
          painter: _AtmosphereEnginePainter(
            ...
            mousePosition: _smoothCursor, // Use interpolated cursor
...
class _StarNode {
  ...
  _StarNode({required this.basePosition, required this.driftSpeed, required this.driftPhase, required this.baseSize});
}
...
class _GeometryNode {
  ...
  final int type;
  final double size;
  _GeometryNode({required this.basePosition, required this.driftSpeed, required this.driftPhase, required this.type, required this.size});
}
...
class _BlueprintPlane {
  ...
  final double driftPhase;
  final double driftSpeed;
  _BlueprintPlane({required this.basePosition, required this.width, required this.height, required this.rotationSpeed, required this.driftPhase, required this.driftSpeed});
}
...
class _ParametricCurve {
  ...
  final double driftSpeed;
  final double length;
  _ParametricCurve({required this.basePosition, required this.amplitude, required this.frequency, required this.phaseOffset, required this.driftSpeed, required this.length});
}
...
class _AtmosphereEnginePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Layer 0.5: Blueprint Planes (floating CAD wireframes)
    _paintBlueprintPlanes(canvas, size);

    // Layer 0.7: Parametric Curves (mathematical structures)
    _paintParametricCurves(canvas, size);

    // Layer 1: Constellation
    _paintConstellation(canvas, size);

    // Layer 2: Geometry / HUD
    _paintGeometry(canvas, size);

    // Layer 3: Particles
    _paintParticles(canvas, size);
    
    // Layer 4: Global Light Source (Enhanced Cursor Glow)
  ...
  void _paintLightingEngine(Canvas canvas, Size size) {
    ...
    // Primary spotlight — tight, follows cursor
    ...
    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height), primaryPaint);

    // Secondary volumetric fog — much larger, creates atmosphere
    ...
    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height), warmPaint);
  }
  ...
  void _paintBlueprintPlanes(Canvas canvas, Size size) {
    ...
    for (var plane in blueprintPlanes) {
      double dx = plane.basePosition.dx + sin(time * plane.driftSpeed + plane.driftPhase) * 10.0 + layerOffset.dx;
      double dy = plane.basePosition.dy + cos(time * plane.driftSpeed + plane.driftPhase) * 10.0 + layerOffset.dy;
      ...
      final double proximityBoost = (1.0 - (distToMouse / 600.0).clamp(0.0, 1.0)) * 0.02;
      ...
      // Internal subdivision lines (architectural feel)
      ...
      // Top-left
      canvas.drawLine(Offset(-hw, -hh + tickLen), Offset(-hw, -hh), paint);
      canvas.drawLine(Offset(-hw, -hh), Offset(-hw + tickLen, -hh), paint);
      // Top-right
      canvas.drawLine(Offset(hw, -hh + tickLen), Offset(hw, -hh), paint);
      canvas.drawLine(Offset(hw, -hh), Offset(hw - tickLen, -hh), paint);
      // Bottom-left
  ...
  void _paintParametricCurves(Canvas canvas, Size size) {
    ...
    for (var curve in parametricCurves) {
      double baseDx = curve.basePosition.dx + sin(time * curve.driftSpeed) * 20.0 + layerOffset.dx;
      double baseDy = curve.basePosition.dy + cos(time * curve.driftSpeed) * 20.0 + layerOffset.dy;
      ...
      for (double t = 0; t <= curve.length; t += 3.0) {
        ...
        final double y = baseDy + sin(t * curve.frequency * 0.02 + time * 0.3 + curve.phaseOffset) * curve.amplitude;
  ...
  void _paintDeepSpace(Canvas canvas, Size size) {
    ...
    for (var node in deepSpaceNodes) {
      double dx = node.basePosition.dx + sin(time * node.driftSpeed + node.driftPhase) * 5.0 + layerOffset.dx;
      double dy = node.basePosition.dy + cos(time * node.driftSpeed + node.driftPhase) * 5.0 + layerOffset.dy;
      
      dy = dy % (size.height * 2);
      if (dy < 0) dy += size.height * 2;
      dy -= size.height * 0.5;

      // Depth-of-field: elements closer to cursor glow slightly brighter
  ...
  void _paintConstellation(Canvas canvas, Size size) {
    ...
    for (var node in constellationNodes) {
      double dx = node.basePosition.dx + sin(time * node.driftSpeed + node.driftPhase) * 15.0 + layerOffset.dx;
      double dy = node.basePosition.dy + cos(time * node.driftSpeed + node.driftPhase) * 15.0 + layerOffset.dy;
    ...
    for (int i = 0; i < currentPositions.length; i++) {
      ...
      final double hoverIntensity = (1.0 - (distanceToMouse / 300)).clamp(0.0, 1.0);
    ...
    for (int i = 0; i < currentPositions.length; i++) {
      ...
      canvas.drawCircle(p, node.baseSize * hoverScale, nodePaint);
    }
  }
  ...
  void _paintGeometry(Canvas canvas, Size size) {
    ...
    for (var node in geometryNodes) {
      double dx = node.basePosition.dx + sin(time * node.driftSpeed + node.driftPhase) * 20.0 + layerOffset.dx;
      double dy = node.basePosition.dy + cos(time * node.driftSpeed + node.driftPhase) * 20.0 + layerOffset.dy;
      ...
      if (node.type == 0) {
        canvas.drawLine(Offset(-node.size/2, 0), Offset(node.size/2, 0), paint);
        canvas.drawLine(Offset(0, -node.size/2), Offset(0, node.size/2), paint);
      } else if (node.type == 1) {
        canvas.drawRect(Rect.fromCenter(center: Offset.zero, width: node.size, height: node.size), paint);
      } else {
        canvas.drawCircle(Offset.zero, node.size/2, paint);
      }
      canvas.restore();
    }
  }
  ...
  void _paintParticles(Canvas canvas, Size size) {
    ...
    for (var node in particleNodes) {
      double dx = node.basePosition.dx + sin(time * node.driftSpeed * 3 + node.driftPhase) * 30.0 + layerOffset.dx;
      double dy = node.basePosition.dy + cos(time * node.driftSpeed * 3 + node.driftPhase) * 30.0 + layerOffset.dy;
      
      dy = dy % (size.height * 2);
      if (dy < 0) dy += size.height * 2;
      dy -= size.height * 0.5;

      // Section specific logic for particles
